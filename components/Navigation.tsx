...
export const Navigation = () => {
  ...
  const isFNFT = useMemo(() => asPath === "/fnft", [asPath]);

  return (
    <nav className="flex h-24 items-center justify-between w-full py-2">
      <div className="flex items-center">
        <Link href="/">
          <button className={twMerge(className, isHome && activeClassName)}>
            home
          </button>
        </Link>

        <Link href="/transact">
          <button className={twMerge(className, isTransact && activeClassName)}>
            transact
          </button>
        </Link>

        <Link href="/sign">
          <button className={twMerge(className, isSign && activeClassName)}>
            Sign
          </button>
        </Link>

        <Link href="/mint">
          <button className={twMerge(className, isMint && activeClassName)}>
            Mint
          </button>
        </Link>
        <Link href="/fnft">
          <button className={twMerge(className, isFNFT && activeClassName)}>
            Fractionalized NFT
          </button>
        </Link>
      </div>
